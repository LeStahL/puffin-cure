     ...
     "else"
       ...
       "else"
         " if(index==4.)"
           "{"
             "x*=.01;"
             "vec3 c1=1.4*stdcolor(150.*x.z+x.xy+.5*rand(ind.xy+47.)+14.),c2=1.4*stdcolor(150.*x.z+x.xy+x.yz+x.zx+.5*rand(ind.xy+12.)+21.+uv),c3=1.4*stdcolor(150.*x.z+x.xy+x.yz+x.zx+.5*rand(ind.xy+15.)+33.+uv);"
             "col=.1*c1*vec3(1.,1.,1.)+.2*c1*vec3(1.,1.,1.)*ln+1.5*vec3(1.,1.,1.)*pow(rev,2.*(2.-1.5*clamp(iScale,0.,1.)))+2.*c1*pow(rev,8.)+3.*c1*pow(rev,16.);"
             "col=clamp(.23*col,0.,1.);"
           "}"
   "return col;"
